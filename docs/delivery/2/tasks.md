...
| :------ | :--- | :----- | :---------- |
| 2-1 | [Backend interface & domain types](./2-1.md) | Done | Define Backend Go interface, WorkloadSpec, WorkloadResult, BackendCapabilities types; add IsolationAuto constant |
| 2-2 | [Workload state machine](./2-2.md) | Done | Add state transition validation to model, enforce in store; invalid transitions return errors |
| 2-3 | [Store extensions for execution](./2-3.md) | Done | Extend Store interface with UpdateWorkload (full update) and GetWorkloadStats; implement in SQLite |
| 2-4 | [Backend registry & auto-routing](./2-4.md) | Done | Backend registry for registration/lookup, auto-mode resolution logic, GET /v1/backends endpoint |
| 2-5 | [Async execution engine](./2-5.md) | Done | Goroutine-based executor with timeout enforcement; POST /v1/workloads/async returning 202; real-time status updates |
| 2-6 | [Log streaming via SSE](./2-6.md) | Done | Log broker with per-workload channels; GET /v1/workloads/:id/logs SSE endpoint |
| 2-7 | [Stats endpoint](./2-7.md) | Done | GET /v1/stats returning aggregate execution statistics per backend |
| 2-8 | [E2E CoS Test](./2-8.md) | Done | End-to-end tests verifying all 9 acceptance criteria for PBI 2 |
