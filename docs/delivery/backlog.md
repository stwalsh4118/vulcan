...
| 10 | User | As a user, I want log lines to stream in real time and SSE connections to clean up automatically so that monitoring workload execution is responsive and reliable | Agreed | Log lines stream to browser in real time (not batched); backend sends explicit "done" SSE event; frontend handles "done" event cleanly (no reconnect loops); no leaked EventSource connections after workload completion; Next.js streaming API route eliminates proxy buffering |
