...
| 4 | User | As a user, I want to run workloads in Firecracker microVMs so that I get hardware-level isolation for arbitrary runtimes and can test this from the frontend | Agreed | Firecracker microVMs boot and execute workloads; CNI networking provides connectivity; vsock transfers code and results; guest agent runs inside VM; rootfs images for Go/Node/Python work; frontend can submit and monitor microVM workloads |
