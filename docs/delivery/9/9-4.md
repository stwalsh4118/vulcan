...
| 2026-02-19 07:28:32 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-19 07:31:56 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
...
- 3 new tests: TestGetLogHistoryHappyPath, TestGetLogHistoryEmpty, TestGetLogHistoryNotFound.
- API spec updated with new endpoint documentation.

## Files Modified

- `api/internal/api/logs.go` — Added `handleGetLogHistory` handler, response types.
- `api/internal/api/server.go` — Registered `GET /{id}/logs/history` route.
- `api/internal/api/logs_test.go` — Added 3 endpoint tests.
- `docs/api-specs/core/core-api.md` — Added endpoint docs, updated Store interface.
